...
func newMinimalNodeConfig(serviceIPCIDR string, nodePortRange string, hostNameOverride string, externalIPs []string) *CopyPasta {
	var config &CopyPasta
	...
	if serviceIPCIDR != "" {
		...
		config.ServiceCIDR = cidr
	} else {
		config.ServiceCIDR = &net.IPNet{}
	}
	if nodePortRange != "" {
		...
		config.ServiceNodePortRange = *portRange
	}
	if hostNameOverride != "" {
		config.NodeName = hostNameOverride
	}
	if externalIPs != nil {
