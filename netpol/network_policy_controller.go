...
import (
	...
	"github.com/manuelbuil/netPolicy/utils"
...
func NewNetworkPolicyController(clientset kubernetes.Interface,
	config *CopyPasta, podInformer cache.SharedIndexInformer,
	...
	npc.serviceClusterIPRange = *config.ServiceCIDR
	npc.serviceNodePortRange = strings.ReplaceAll(config.ServiceNodePortRange.String(), "-", ":")
	npc.syncPeriod = defaultSyncPeriod

	node, err := utils.GetNodeObject(clientset, config.NodeName)
	if err != nil {
		return nil, err
	}
